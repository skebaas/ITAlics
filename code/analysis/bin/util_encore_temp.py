...
def load_design_matrix_new(mat_files, sequence):
    """
    Read in 'mat_files' which is a list containing the paths to mulitple .mat files. 
    Select the design matrix that corresponds to the 'sequence' identifier.
    
    Use `nipype.interfaces.base import Bunch` to return a Bunch object that can be used by `nipype.interfaces.spm.SpecifyModel`.
    
    Parameters:
        mat_files (list): a list of paths to .mat files.
        sequence (str): sequence identifier
    Returns:
        A Bunch object, where each Bunch corresponds to a loaded design matrix.
        Each Bunch has the following attributes:
        conditions(list): a list of condition names
        onsets(list): a list of lists of onset times
        durations(list): a list of lists of durations
    ...
    from nipype.interfaces.base import Bunch
    import scipy.io as sp
    # Return a list of Bunch objects, where each Bunch corresponds to a loaded design matrix.
    # Select the design matrix that corresponds to the 'sequence' identifier.
    for mat in mat_files:
        if sequence in mat:
            mat_file = load_design_matrix(mat)
    
    # convert dm to Bunch object with conditions, onsets, durations, pmod
    # load dm with sp.loadmat
    dm = sp.loadmat(mat_file, squeeze_me=True)
    names = dm.get('names').tolist()
    onsets = dm.get('onsets').tolist()
    durations = dm.get('durations').tolist()
...
def datasource(directory, sequence, session=1, BHV_struct='BHV*/Scan'):
    ...
    # If the sequence is a task-specific scan, add behavior files, mask file, and motion parameters to the output fields
    elif ('reward', 'efnback', 'dynface', 'paat') in sequence:
        behavior_files = {
        ...
        for behavior_file_pattern in behavior_file_patterns:
            ...
            field_template['behav'] = f"BHV*/{behavior_file_pattern}"
        ...
        #field_template['behav'] = f"BHV*/PAAT_*m*.mat"
        #template_args['behav'] = [[]]
        #outfields.append('behav')
